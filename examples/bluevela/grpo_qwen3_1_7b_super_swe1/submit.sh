...
readonly target_test="tests/test_suites/llm/grpo-qwen3-1.7b-1n8g-megatron-super-swe1.sh"
...
Usage: submit.sh [--dry-run|--submit]
  ...
  --dry-run  Print the resolved run layout and both LSF submissions. This is
             the default and does not create the run directory.
  --submit   Create a unique run directory and submit preparation followed by
             a done(prep_job_id)-dependent 1-node/8-H100 training job.
EOF
}
...
cache_root=${CACHE_ROOT:-${default_storage_root}/cache/nemo-rl-super-swe1}
...
run_id=${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-grpo-qwen3-1p7b-super-swe1-yuetai}
run_dir=${RUN_DIR:-${run_root}/nemo-rl-super-swe1/${run_id}}
...
train_slots=${TRAIN_SLOTS:-8}
train_memory_mb=${TRAIN_MEMORY_MB:-524288}
train_walltime=${TRAIN_WALLTIME:-03:00}
gpu_requirement=${GPU_REQUIREMENT:-num=8:mode=shared:j_exclusive=yes}
prep_job_name=${PREP_JOB_NAME:-nrl-super-swe1-prep-yuetai}
train_job_name=${TRAIN_JOB_NAME:-nrl-super-swe1-train-yuetai}
...
[[ $train_slots == 8 ]] || die "TRAIN_SLOTS must remain 8"
[[ $gpu_requirement == num=8:mode=shared:j_exclusive=yes ]] \
    || die "GPU_REQUIREMENT must use job-exclusive GPUs in shared compute mode"

prep_resource="span[hosts=1] select[tmp>${prep_tmp_mb}] rusage[mem=${prep_memory_mb}]"
train_resource="span[hosts=1] rusage[mem=${train_memory_mb}]"
...
render() {
    ...
    echo "Training resources: -q ${lsf_queue} -G ${lsf_group} -n ${train_slots} -M ${train_memory_mb} -W ${train_walltime} -gpu ${gpu_requirement}"
...
git -C "$repo_dir" cat-file -e "${source_commit}:${target_test}" \
    || die "target test does not exist in SOURCE_COMMIT ${source_commit}"
validate_secret_file WANDB_API_KEY_FILE "$wandb_key_file"
validate_secret_file HF_TOKEN_FILE "$hf_token_file"

mkdir -p \
    "$run_dir/control" "$run_dir/logs" "$run_dir/status" "$run_dir/tmp" \
    "$run_dir/home" "$run_dir/wandb" "$cache_root" "$(dirname -- "$container_image")"
cp "$script_dir/prepare.sh" "$script_dir/train.sh" "$run_dir/control/"
chmod 700 "$run_dir/control/prepare.sh" "$run_dir/control/train.sh"

{
    ...
    write_export TARGET_TEST "$target_test"
} >"$run_dir/control/run.env"
chmod 600 "$run_dir/control/run.env"
sha256sum "$run_dir/control/prepare.sh" "$run_dir/control/train.sh" \
...
[[ -n $prep_job_id ]] || die "unable to parse preparation job ID from: ${prep_output}"
...
[[ -n $train_job_id ]] || die "unable to parse training job ID from: ${train_output}"
